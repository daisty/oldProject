var app = angular.module('setting', [{
    files:[
        "root/financeinit/setting/_res/js/service.js"
    ]
}]);
app.controller('settingCtrl',function ($scope,$state) {

    if ($state.current.url == '/setting') {//默认加载列表
        $state.go('root.checkfunds.setting.list[12]')
    }
    ...
    $scope.list = function(){
        $state.go('root.financeinit.setting.list[12]');
        $scope.menuClass = 'listMenu'
    };
    $scope.edit = function(){
        if($scope.editId){
            $state.go('root.financeinit.setting.edit[12]',{id:$scope.editId,page:$scope.page});
