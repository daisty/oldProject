var app = angular.module('qulist', ['ng-pagination','toastr']);
app.controller('qulistCtrl',function($scope,accountancourseSer,toastr,$state,$location,$stateParams){
    ...
    function activatePage(page) {
        ...
        accountancourseSer.qulistList(listData).then(function(response){
            if(response.data.code==0){
    ...
    // 列表分页
    accountancourseSer.countQulist().then(function(response){
        if(response.data.code==0){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.financeinit.accountancourse.qulist[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        accountancourseSer.deleteCarInsurance(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.custom.itemsCount-count)%10){
                    ...
                    $state.go('root.financeinit.accountancourse.qulist[12]',{id:null,name:null,page:$location.search().page-1});
                }
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
