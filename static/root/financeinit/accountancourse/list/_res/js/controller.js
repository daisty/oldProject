var app = angular.module('accountancourseList', ['ng-pagination', 'toastr']);
app.controller('accountancourseListCtrl', function ($scope, accountancourseSer, toastr, $state, $location, $stateParams) {
    $scope.$emit('changeId', null); //列表id
    ...
    function activatePage(page) {
        ...
        accountancourseSer.carInsuranceList(listData).then(function (response) {
            if (response.data.code == 0) {
                ...
                if ($stateParams.id) {
                    ...
                    $scope.$emit('oneLevelId', $stateParams.id);
                    $scope.$emit('twiceLevelId', $stateParams.id);
                    $scope.$emit('threeLevelId', $stateParams.id);
                }
            } else {
                toastr.error(response.data.msg, '温馨提示');
            }
    ...
    $scope.selectList = function (event) {
        ...
        if ($scope.twoLists != []) {
            $scope.cat = $scope.mailLists;
        }else {
            $scope.cat = $scope.mailLists.concat($scope.twoLists);
        }

        angular.forEach($scope.cat, function (obj) {
    ...
    // 列表分页
    accountancourseSer.countcarInsurance().then(function (response) {
        if (response.data.code == 0) {
    ...
    $scope.cancel = function () {//取消删除
        $scope.delShow = false;
        $state.go('root.financeinit.accountancourse.list[12]', { id: null, name: null });
    };
    var count = 0;
    $scope.delFn = function () {//确认删除
        ...
        accountancourseSer.deleteCarInsurance(data).then(function (response) {
            if (response.data.code == 0) {
                ...
                $scope.$emit('oneLevelId', null);
                $scope.$emit('twiceLevelId', null);
                $scope.$emit('thirdlevelId', null);
                $scope.$emit('ziId', null); //资产id
                $scope.delShow = false;
                if (($scope.custom.itemsCount - count) % 10) {
                    $state.go('root.financeinit.accountancourse.list[12]', { id: null, name: null });
                } else {
                    $state.go('root.financeinit.accountancourse.list[12]', { id: null, name: null, page: $location.search().page - 1 });
                }
            } else {
                toastr.error(response.data.msg, '温馨提示');
            }
